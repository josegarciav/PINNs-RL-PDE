...
class FiniteDifferenceSolver(ABC):
    ...
    def __init__(self, config: FDMConfig):
        ...
        self.dx = (
            config.dx[0]
            if hasattr(config, "dx")
            else (config.x_domain[1] - config.x_domain[0]) / (config.nx - 1)
        )
        self.dt = (
            config.dt
            if hasattr(config, "dt")
            else (config.t_domain[1] - config.t_domain[0]) / (config.nt - 1)
        )
    ...
    def plot_analytical_comparison(
        self, analytical_func, save_path=None, num_time_steps=4
    ):
        """
    ...
    def plot_solution_3d(self, save_path=None, title=None, figsize=(10, 8)):
        ...
        try:
            ...
            fig.update_layout(
                title=dict(
                    text=plot_title, x=0.5, xanchor="center", font=dict(size=18)
                ),
                scene=dict(
                    xaxis_title="x",
                    yaxis_title="t",
    ...
    def plot_comparison_with_pinn(
        self, pinn_model, save_path=None, device="cpu", title=None, figsize=(12, 10)
    ):
        """
        ...
        try:
            ...
            t_indices = np.linspace(
                0, self.config.nt - 1, 4, dtype=int
            )  # Select 4 time points
            t_selected = np.array(
                [self.config.t_domain[0] + idx * self.dt for idx in t_indices]
            )
    ...
    def _plot_heatmap_comparison(self, ax, pinn_model, device="cpu"):
        ...
        gs = GridSpecFromSubplotSpec(
            1, 3, subplot_spec=ax.get_subplotspec(), width_ratios=[1, 1, 0.05]
        )
        ...
        # Plot FDM solution
        im1 = ax1.pcolormesh(
            X, T, fdm_sol, shading="auto", cmap="viridis", vmin=vmin, vmax=vmax
        )
        ...
        # Plot PINN solution
        im2 = ax2.pcolormesh(
            X, T, pinn_sol, shading="auto", cmap="viridis", vmin=vmin, vmax=vmax
        )
