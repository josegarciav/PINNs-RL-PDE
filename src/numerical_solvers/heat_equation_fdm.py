...
class HeatEquationFDM:
    ...
    def _evaluate_pinn_full(self, model, device):
        ...
        for t in t_tensor:
            ...
            with torch.no_grad():
                predictions = (
                    model(points).cpu().numpy().squeeze()
                )  # Add squeeze() to remove extra dimension
    ...
    def generate_fdm_comparison_plots(pde, model, device, viz_dir, logger=None):
        ...
        try:
            ...
            x = torch.linspace(
                solver.pde.domain[0][0],
                solver.pde.domain[0][1],
                solver.nx,
                device=device,
            )
            t = torch.linspace(
                solver.pde.time_domain[0],
                solver.pde.time_domain[1],
                solver.nt,
                device=device,
            )
            x_grid, t_grid = torch.meshgrid(x, t, indexing="ij")
            ...
            with torch.no_grad():
                ...
                u_exact = solver.pde.exact_solution(
                    points[:, 0].reshape(-1, 1), points[:, 1].reshape(-1, 1)
                )
            ...
            solver.plot_solution(
                model=model,
                save_path=os.path.join(viz_dir, "fdm_solution.png"),
                device=device,
            )

            # Plot comparison with PINN and exact solution
            solver.plot_comparison_with_pinn(
                model,
            ...
            logger.info(
                f"FDM vs Exact - L2={metrics['exact_fdm_l2_error']:.6f}, Max={metrics['exact_fdm_max_error']:.6f}, Mean={metrics['exact_fdm_mean_error']:.6f}"
            )
