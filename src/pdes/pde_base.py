...
class PDEConfig:
    ...
    name: str
    domain: Union[Tuple[float, float], List[Tuple[float, float]]]  # For 1D, list of tuples for higher dimensions
...
class PDEBase:
    ...
    def generate_collocation_points(self, num_points: int, strategy: str = 'uniform') -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        if strategy == 'uniform':
            if self.dimension == 1:
                x = torch.linspace(self.domain[0], self.domain[1], num_points).reshape(-1, 1)
            else:
                # Generate points for each dimension
                x = []
                for dim in range(self.dimension):
                    x.append(torch.linspace(self.domain[dim][0], self.domain[dim][1], num_points))
                x = torch.stack(x, dim=1)
            
            t = torch.linspace(self.config.time_domain[0], self.config.time_domain[1], num_points).reshape(-1, 1)
            ...
            x = []
            for dim in range(self.dimension):
                x.append(torch.tensor(qmc.scale(sample[:, [dim]], 
            ...
            t = torch.tensor(qmc.scale(sample[:, [-1]], 
            ...
            while len(points) < num_points:
                ...
                if self.dimension == 1:
                    ...
                    x = []
                    for dim in range(self.dimension):
                        x.append(torch.rand(1, 1) * (self.domain[dim][1] - self.domain[dim][0]) + self.domain[dim][0])
                    x = torch.cat(x, dim=1)
                
                t = torch.rand(1, 1) * (self.config.time_domain[1] - self.config.time_domain[0]) + self.config.time_domain[0]
    ...
    def compute_loss(self, model: torch.nn.Module, x: torch.Tensor, t: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        ...
        u_boundary_exact = torch.cat([
            self.boundary_conditions['left'](x_boundary[0:1], t_boundary[0:1]),
            self.boundary_conditions['right'](x_boundary[1:2], t_boundary[1:2])
        ])
    ...
    def plot_solution(self, model: torch.nn.Module, num_points: int = 1000, save_path: Optional[str] = None):
        """
