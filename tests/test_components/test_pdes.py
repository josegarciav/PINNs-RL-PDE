...
class TestPDEs(unittest.TestCase):
    ...
    def test_heat_equation(self):
        ...
        self.assertTrue(torch.all(t >= config.time_domain[0]) and torch.all(t <= config.time_domain[1]))
        ...
        x_boundary = torch.tensor([config.domain[0][0], config.domain[0][1]], 
        ...
        config_2d = PDEConfig(
            ...
            initial_condition={"type": "sine_2d", "amplitude": 1.0, "frequency_x": 2.0, "frequency_y": 2.0},
            exact_solution={"type": "sine_2d", "amplitude": 1.0, "frequency_x": 2.0, "frequency_y": 2.0},
            dimension=2,
            device=self.device
        )
