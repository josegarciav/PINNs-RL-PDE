...
class TestPDEs(unittest.TestCase):
    ...
    def test_heat_equation(self):
        ...
        x_boundary = torch.tensor([domain[0][0], domain[0][1]], dtype=torch.float32).reshape(
            -1, 1
        )
        t_boundary = torch.zeros_like(x_boundary)
        u_boundary = heat_eq._create_boundary_condition("initial", initial_condition)(
    ...
    def test_wave_equation(self):
        ...
        t_boundary = torch.zeros_like(x_boundary)
        u_boundary = wave_eq._create_boundary_condition("initial", initial_condition)(
            x_boundary, t_boundary
        )
        self.assertEqual(u_boundary.shape, (2, 1))
    ...
    def test_burgers_equation(self):
        ...
        domain = [(-1.0, 1.0)]  # Changed to list of tuples
        time_domain = (0.0, 1.0)
        boundary_conditions = {
        ...
        t_boundary = torch.zeros_like(x_boundary)
        u_boundary = burgers._create_boundary_condition("initial", initial_condition)(
            x_boundary, t_boundary
        )
        self.assertEqual(u_boundary.shape, (2, 1))
    ...
    def test_pendulum_equation(self):
        ...
        # Test exact solution
        u_exact = pde.compute_exact_solution(x, t)
        assert u_exact.shape == (100, 1)
        assert torch.allclose(
            u_exact,
        ...
        u = torch.sin(x + t)
        residual = pde.compute_residual(x, t, u)
        assert residual.shape == (100, 1)

        # Test time derivative
        du_dt = pde.compute_time_derivative(u, t, order=1)
        d2u_dt2 = pde.compute_time_derivative(u, t, order=2)
        assert du_dt.shape == (100, 1)
        assert d2u_dt2.shape == (100, 1)
        assert torch.allclose(du_dt, torch.cos(x + t), atol=1e-6)
        assert torch.allclose(d2u_dt2, -torch.sin(x + t), atol=1e-6)

        # Test energy computation
        energy = pde.compute_energy(x, t, u)
        assert energy.shape == (100, 1)
        assert torch.all(energy >= 0)  # Energy should be non-negative

        # Test phase space computation
        phase_space = pde.compute_phase_space(x, t, u)
        ...
        with pytest.raises(ValueError):
            pde.compute_exact_solution(x, t)
