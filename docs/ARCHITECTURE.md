...
The seven architectures are registered in `pinnrl/neural_networks/__init__.py` and selected via the `architecture` field in `ModelConfig` or `config.yaml`.

### FeedForward

**File:** `pinnrl/neural_networks/feedforward.py`
...
**File:** `pinnrl/neural_networks/resnet.py`
...
**File:** `pinnrl/neural_networks/siren.py`
...
**File:** `pinnrl/neural_networks/fourier.py`
...
**File:** `pinnrl/neural_networks/attention.py`
...
**File:** `pinnrl/neural_networks/autoencoder.py`
...
When `rl.enabled: true` in `config.yaml`, the `CollocationRLAgent` (implemented in `pinnrl/rl/rl_agent.py`) replaces uniform random collocation sampling with a DQN-based point selection strategy.
...
Set `training.adaptive_weights.enabled: true` to use the `AdaptiveLossWeights` module (`pinnrl/components/adaptive_weights.py`). Two strategies are available:
...
`Config.__init__()` reads `pinnrl/config/config.yaml` and builds a `ModelConfig` dataclass that is passed to `PINNModel`. The mapping is:
...
All architecture classes inherit from `BaseNetwork` (`pinnrl/neural_networks/base_network.py`), which standardizes the `forward(x: Tensor) -> Tensor` interface and provides shared utilities for weight initialization and parameter counting.
